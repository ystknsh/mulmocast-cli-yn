...
const main = async () => {
    const { o: outdir, t: template, u: urls, b: basedir, action, v: verbose, i: interactive } = tool_args_1.args;
    ...
    if (action === "scripting") {
        if (interactive) {
            await (0, seed_1.createMulmoScriptWithInteractive)({ outdir: outDirPath });
        }
        else if (urls) {
