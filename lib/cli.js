...
const main = async () => {
    const { outdir, basedir, file } = args_1.args;
    const baseDirPath = getBaseDirPath(basedir);
    const mulmoFilePath = getFullPath(baseDirPath, file ?? "");
    const outFilePath = getFullPath(baseDirPath, outdir ?? "output");
    if (args_1.args.v) {
    ...
    const studio = (0, preprocess_1.createOrUpdateStudioData)(mulmoFilePath);
    if (action === "translate") {
        await (0, translate_1.translate)(studio, { outFilePath });
    }
    if (action === "audio") {
        await (0, audio_1.audio)(studio, methods_1.MulmoScriptMethods.getSpeechProvider(studio.script) === "nijivoice" ? 1 : 8);
    }
    if (action === "images") {
