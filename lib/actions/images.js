...
const preprocess_agent = async (namedInputs) => {
    ...
    if (beat.media) {
        if (beat.media.type === "textSlide") {
            ...
            await (0, markdown_1.convertMarkdownToImage)(markdown, methods_1.MulmoScriptMethods.getTextSlideStyle(studio.script, beat), imagePath);
        }
        else if (beat.media.type === "markdown") {
            const markdown = beat.media.markdown.join("\n");
            // NOTE: If we want to support per-beat CSS style, we need to add textSlideParams to MulmoBeat and process it here.
            await (0, markdown_1.convertMarkdownToImage)(markdown, methods_1.MulmoScriptMethods.getTextSlideStyle(studio.script, beat), imagePath);
        }
    }
