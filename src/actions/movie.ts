  ...
  studio.beats.reduce((timestamp, studioBeat, index) => {
    ...
    if (studio.script.movieParams?.transition && index < studio.beats.length - 1) {
      ...
      if (mediaType === "movie") {
        ...
        ffmpegContext.filterComplex.push(`[${sourceId}_1]reverse,select='eq(n\,0)',reverse,tpad=stop_mode=clone:stop_duration=${duration},setpts=PTS-STARTPTS[${sourceId}_2]`);
