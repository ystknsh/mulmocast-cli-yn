...
test("test getVideoParts image", async () => {
  const { videoPart } = getVideoPart(1, "image", 200, { width: 100, height: 300 }, { style: "aspectFit" });
  assert.equal(
    videoPart,
...
test("test getVideoParts movie", async () => {
  const { videoPart } = getVideoPart(1, "movie", 200, { width: 100, height: 300 }, { style: "aspectFit" });
  assert.equal(
    videoPart,
